...
Class MessageReadTable extends Entity\DataManager
{
    ...
    public static function getLastUnreadId($ticketId)
    {
        ...
        if($data = self::getRow(array('filter'=>array('USER_ID'=>$USER->GetId(),'TICKET_ID'=>$ticketId),'select'=>array('LAST_MESSAGE_ID'))))
        {
    ...
    {
        if($data = self::getRow(array('filter'=>array('USER_ID'=>$fields['USER_ID'],'TICKET_ID'=>$fields['TICKET_ID']),'select'=>array('ID'))))
        {
