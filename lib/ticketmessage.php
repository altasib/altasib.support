    ...
    public static function add($data)
    {
        ...
        if($result->isSuccess())
        {
            ...
            if(!defined('SUP_IMPORT'))
            {
                ...
                {
                    //calc
                    if(\Bitrix\Main\Loader::includeModule("altasib.supportreport") && (int)$data['ELAPSED_TIME']>0 && (int)$data['TYPE_TIME_ID']>0)
                    {
                        $cost = \ALTASIB\SupportReport\Cost::calcByMessageId($data['MESSAGE_ID']);
                        self::update($data['MESSAGE_ID'],$cost);
                    }
