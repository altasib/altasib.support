...
class TicketTable extends Entity\DataManager
{
	...
	public static function getMap()
	{
        ...
        $map = array(
                ...
                'TIMESTAMP_X' => array(
                        'data_type' => 'datetime',
                        'expression' => array(
                                $DB->DatetimeToDateFunction('%s'), 'TIMESTAMP'
                        )
                ),                
                ...
                'DATE_CLOSE_X' => array(
                        'data_type' => 'datetime',
                        'expression' => array(
                                $DB->DatetimeToDateFunction('%s'), 'DATE_CLOSE'
                        )
                ),                
    ...
    public static function add(array $data)
    {
        ...
        {
            $category = Support\CategoryTable::getList(array('filter'=>array('USE_DEFAULT'=>'Y'),'select'=>array('ID')));
        ...
        //check personal resp..
        $dataClient = ClientTable::getList(array('filter'=>array("USER_ID" => $data['OWNER_USER_ID'],'>RESPONSIBLE_USER_ID'=>0),'select'=>array('RESPONSIBLE_USER_ID')));
        ...
        {
            $category = CategoryTable::getList(array('filter'=>array('ID'=>$data['CATEGORY_ID']),'select'=>array('RESPONSIBLE_USER_ID')));
        ...
        if(!isset($data['SLA_ID']))
        {
            ...
            $dataSlaGroup = SlaTable::getList(array('filter'=>array('SLA_GROUP.GROUP_ID'=>$userGroups),'select'=>array('GROUP_'=>'SLA_GROUP')));
    ...
    public static function close($ID,$MESSAGE,$changeStatus = true)
    {
        ...
        if(self::isClose($ID))
        {
            self::update($ID,array('IS_CLOSE'=>'N','MODIFIED_USER_ID'=>$USER->GetID(),'TIMESTAMP'=>new Type\DateTime(),'STATUS_ID'=>\Bitrix\Main\Config\Option::get('altasib.support','RE_STATUS',1)));
        ...
        else
        {
            if($changeStatus)
            {
                self::update($ID,array('IS_CLOSE'=>'Y','MODIFIED_USER_ID'=>$USER->GetID(),'TIMESTAMP'=>new Type\DateTime(),'STATUS_ID'=>\Bitrix\Main\Config\Option::get('altasib.support','FINAL_STATUS',1)));
                Event::changeStatus($ID);
            }
            else
    ...
    public static function getRight($ID)
    {
        ...
        else
        {
            ...
            $obTicket = self::getList(array('filter'=>array('ID'=>$ID),'select'=>array("ID",'CATEGORY_ID',"OWNER_USER_ID","RESPONSIBLE_USER_ID")));
            ...
            else
            {
                ...
                if(!$isWorker)
                {
                    if($arTicket['OWNER_USER_ID'] != $USER->GetID())
                    {
                        $c2cw = C2CWTable::GetList(
                            array(
                                'filter' => array('USER_ID'=>$USER->GetId(),'WORKER_USER_ID'=>$arTicket['OWNER_USER_ID'])
                            )
                        );
                        if(!$c2cw->fetch())
                        {
    ...
    public static function autoClose()
    {
        ...
        if($dayLeft>0)
        {
            ...
            $data = self::getList(array(
                ...
                'filter' => array('IS_CLOSE'=>'N','IS_DEFERRED'=>'N','<LAST_MESSAGE_DATE'=>new Type\DateTime(date('Y-m-d H:i:s',$ts),'Y-m-d H:i:s'),'CATEGORY.NOT_CLOSE'=>'N')
            ));
    ...
    public static function checkOverdue()
    {
        ...
        $data = self::getList(array(
            ...
            'select' => array(
                'ID','DATE_CREATE','LAST_MESSAGE_USER_ID','LAST_MESSAGE_DATE','SLA_RESPONSE_TIME'=>'SLA.RESPONSE_TIME'
            ),
        ));
        while($ticket = $data->fetch())
        {
            ...
            //todo: to sql
            $ts1 = \AddToTimeStamp(array('HH'=>$ticket['SLA_RESPONSE_TIME']),\MakeTimeStamp($ticket['LAST_MESSAGE_DATE']->toString()));
            $date = new Type\Date();
    ...
    public static function getTicketRight($ticketId,$ownerId,$responsibleId,$categoryId)
    {
        ...
        if($Role == 'C')
        {
            ...
            $c2cw = \ALTASIB\Support\C2CWTable::GetList(
                array('filter' => array('WORKER_USER_ID'=>$USER->GetID(),'CATEGORY_ID'=>$categoryId,'R_VIEW'=>'Y'))
            );
        ...
        if($Role == 'E')
        {
            ...
            else
            {
                $wtc = \ALTASIB\Support\WtCTable::GetList(
                    array('filter' => array('USER_ID'=>$USER->GetID(),'CATEGORY_ID'=>$categoryId,'CLIENT_USER_ID'=>$ownerId,'R_VIEW'=>'Y'))
                );
                if($client = $wtc->fetch())
                {
                ...
                {
                    if(\ALTASIB\Support\TicketMemberTable::getList(array('select'=>array('ID'),'filter'=>array('USER_ID'=>$USER->GetID(),'TICKET_ID'=>$ticketId)))->fetch())
                    {
                        $right['VIEW'] = true;
                        $right['ANSWER'] = true;
                    }
