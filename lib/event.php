...
class Event
{
     ...
     * @return
     */
    function sendToResponsible($TICKET_ID)
    {
        if($arEvent = self::getEventDataEx($TICKET_ID))
        {
            if(Main\Loader::includeModule("im") && \CIMSettings::GetNotifyAccess($arEvent['TICKET_RESPONSIBLE_USER_ID'], self::$moduleId, 'create', \CIMSettings::CLIENT_SITE))
            {
                \CIMNotify::add(array(
                    ...
                    'NOTIFY_MESSAGE' => Loc::getMessage('ALTASIB_SUPPORT_EVENT_NEW_TICKET',array('#ID#'=>$TICKET_ID,'#TITLE#'=>$arEvent['TICKET_TITLE'],'#MESSAGE#'=>$arEvent['TICKET_MESSAGE'],'#URL#'=>$arEvent['URL'])),
                    'NOTIFY_MESSAGE_OUT' => Loc::getMessage('ALTASIB_SUPPORT_EVENT_NEW_TICKET',array('#ID#'=>$TICKET_ID,'#TITLE#'=>$arEvent['TICKET_TITLE'],'#MESSAGE#'=>$arEvent['TICKET_MESSAGE'],'#URL#'=>$arEvent['URL'])),
                ));                
            }
     ...
     * @return
     */
    function sendNotifyMessageClient($fields)
    {
        global $USER;
        if($arEvent = self::getEventDataEx($fields['TICKET_ID'],$fields['MESSAGE_ID']))
        {
            ...
            foreach($listToNotify as $TO_USER_ID)
            {
                ...
                if(Main\Loader::includeModule("im") && \CIMSettings::GetNotifyAccess($TO_USER_ID, self::$moduleId, 'message', \CIMSettings::CLIENT_SITE))
                {
                    ...
                    \CIMNotify::add(array(
                        ...
                        'NOTIFY_MESSAGE' => Loc::getMessage('ALTASIB_SUPPORT_EVENT_NEW_MESSAGE_CLIENT',array('#ID#'=>$fields['TICKET_ID'],'#TITLE#'=>$arEvent['TICKET_TITLE'],'#MESSAGE#'=>$fields['MESSAGE'],'#URL#'=>$arEvent['URL'])),
                        'NOTIFY_MESSAGE_OUT' => Loc::getMessage('ALTASIB_SUPPORT_EVENT_NEW_MESSAGE_CLIENT',array('#ID#'=>$fields['TICKET_ID'],'#TITLE#'=>$arEvent['TICKET_TITLE'],'#MESSAGE#'=>$fields['MESSAGE'],'#URL#'=>$arEvent['URL'])),
                    ));                    
                }
                $arEvent['FILES'] = self::getFileTxtLine($fields['TICKET_ID'],$fields['MESSAGE_ID'],$arEvent['SITE_ID']);
                ...
                if(Main\Loader::includeModule("altasib.supportreport"))
                {
                    \ALTASIB\SupportReport\SonetNotify::AddMessage(array(
                        'TITLE' => Loc::getMessage('ALTASIB_SUPPORT_EVENT_CHANGE_TICKET',array('#ID#'=>$fields['TICKET_ID'])),
                        'URL' => $arEvent['URL'],
     ...
     * @return
     */
    function sendNotifyMessageSupport($fields)
    {
        global $USER;
        ...
        if($arEvent = self::getEventDataEx($fields['TICKET_ID'],$fields['MESSAGE_ID']))
        {
            ...
            foreach($listToNotify as $TO_USER_ID)
            {
                ...
                if(in_array($TO_USER_ID,$additionalListToNot) || ((in_array($TO_USER_ID,$adminList) && $TO_USER_ID==$arEvent['TICKET_RESPONSIBLE_USER_ID']) || !in_array($TO_USER_ID,$adminList)))
                {
                    if(Main\Loader::includeModule("im") && \CIMSettings::GetNotifyAccess($TO_USER_ID, self::$moduleId, 'message', \CIMSettings::CLIENT_SITE))
                    {
                        \CIMNotify::add(array(
                            ...
                            'NOTIFY_MESSAGE' => Loc::getMessage('ALTASIB_SUPPORT_EVENT_NEW_MESSAGE_SUPPORT',array('#ID#'=>$fields['TICKET_ID'],'#TITLE#'=>$arEvent['TICKET_TITLE'],'#MESSAGE#'=>$fields['MESSAGE'],'#URL#'=>$arEvent['URL'],'#CLOSE_TXT#'=>$arEvent['CLOSE_TXT'])),
                            'NOTIFY_MESSAGE_OUT' => Loc::getMessage('ALTASIB_SUPPORT_EVENT_NEW_MESSAGE_SUPPORT_OUT',array('#ID#'=>$fields['TICKET_ID'],'#TITLE#'=>$arEvent['TICKET_TITLE'],'#MESSAGE#'=>$fields['MESSAGE'],'#URL#'=>$arEvent['URL'])),
                        ));                        
                    }
                }                                    
    ...
    function changeCaregory($TICKET_ID)
    {
        if($arEvent = self::getEventDataEx($TICKET_ID))
        {
            TicketMessageTable::add(Array(
                ...
                'TICKET_ID' => $TICKET_ID,
                'MESSAGE' => Loc::getMessage('ALTASIB_SUPPORT_EVENT_CHANGE_CATEGORY_LOG',array('#CATEGORY#'=>$arEvent['TICKET_CATEGORY'])),
                'IS_LOG' => 'Y'
            ));
            ...
            foreach($listToNotify as $TO_USER_ID)
            {
                if(Main\Loader::includeModule("im") && \CIMSettings::GetNotifyAccess($TO_USER_ID, self::$moduleId, 'edit', \CIMSettings::CLIENT_SITE))
                {
                    ...
                    \CIMNotify::add(array(
                        ...
                        'NOTIFY_MESSAGE' => Loc::getMessage('ALTASIB_SUPPORT_EVENT_CHANGE_CATEGORY',array('#ID#'=>$TICKET_ID,'#CATEGORY#'=>$arEvent['TICKET_CATEGORY'],'#URL#'=>$arEvent['URL'])),
                        'NOTIFY_MESSAGE_OUT' => Loc::getMessage('ALTASIB_SUPPORT_EVENT_CHANGE_CATEGORY',array('#ID#'=>$TICKET_ID,'#CATEGORY#'=>$arEvent['TICKET_CATEGORY'],'#URL#'=>$arEvent['URL'])),
                    ));                                        
                }
                
                $arEvent['MESSAGE'] = self::convertText(Loc::getMessage('ALTASIB_SUPPORT_EVENT_CHANGE_CATEGORY_MAIL',array('#ID#'=>$TICKET_ID,'#CATEGORY#'=>$arEvent['TICKET_CATEGORY'],'#URL#'=>$arEvent['URL']))); 
    ...
    function changeStatus($TICKET_ID)
    {
        ...
        if($arEvent = self::getEventDataEx($TICKET_ID))
        {
            TicketMessageTable::add(Array(
                'CREATED_USER_ID' => $arEvent['TICKET_MODIFIED_USER_ID'],
                'TICKET_ID' => $TICKET_ID,
                'MESSAGE' => Loc::getMessage('ALTASIB_SUPPORT_EVENT_CHANGE_STATUS_LOG',array('#STATUS#'=>$arEvent['TICKET_STATUS'])),
                'IS_LOG' => 'Y'
            ));
            ...
            foreach($listToNotify as $TO_USER_ID)
            {
                ...
                if(Main\Loader::includeModule("im") && \CIMSettings::GetNotifyAccess($TO_USER_ID, self::$moduleId, 'edit', \CIMSettings::CLIENT_SITE))
                {
                    ...
                    \CIMNotify::add(array(
                        ...
                        'NOTIFY_MESSAGE' => Loc::getMessage('ALTASIB_SUPPORT_EVENT_CHANGE_STATUS',array('#ID#'=>$TICKET_ID,'#STATUS#'=>$arEvent['TICKET_STATUS'],'#URL#'=>$arEvent['URL'])),
                        'NOTIFY_MESSAGE_OUT' => Loc::getMessage('ALTASIB_SUPPORT_EVENT_CHANGE_STATUS',array('#ID#'=>$TICKET_ID,'#STATUS#'=>$arEvent['TICKET_STATUS'],'#URL#'=>$arEvent['URL'])),
                    ));                                        
                }
                $arEvent['MESSAGE'] = self::convertText(Loc::getMessage('ALTASIB_SUPPORT_EVENT_CHANGE_STATUS_MAIL',array('#ID#'=>$TICKET_ID,'#STATUS#'=>$arEvent['TICKET_STATUS'],'#URL#'=>$arEvent['URL']))); 
    ...
    function changePriority($TICKET_ID,$last)
    {
        if($arEvent = self::getEventDataEx($TICKET_ID))
        {
            TicketMessageTable::add(Array(
                'CREATED_USER_ID' => $arEvent['TICKET_MODIFIED_USER_ID'],
                'TICKET_ID' => $TICKET_ID,
                'MESSAGE' => Loc::getMessage('ALTASIB_SUPPORT_EVENT_CHANGE_PRIORITY_LOG',array('#PRIORITY#'=>$arEvent['TICKET_PRIORITY'])),
                'IS_LOG' => 'Y'
            ));
            ...
            foreach($listToNotify as $TO_USER_ID)
            {
                ...
                if(Main\Loader::includeModule("im") && \CIMSettings::GetNotifyAccess($TO_USER_ID, self::$moduleId, 'edit', \CIMSettings::CLIENT_SITE))
                {
                    ...
                    \CIMNotify::add(array(
                        ...
                        'NOTIFY_MESSAGE' => Loc::getMessage($messId,array('#ID#'=>$TICKET_ID,'#PRIORITY#'=>$arEvent['TICKET_PRIORITY'],'#URL#'=>$arEvent['URL'])),
                        'NOTIFY_MESSAGE_OUT' => Loc::getMessage($messId,array('#ID#'=>$TICKET_ID,'#PRIORITY#'=>$arEvent['TICKET_PRIORITY'],'#URL#'=>$arEvent['URL'])),
                    ));                    
                }

                $arEvent['MESSAGE'] = self::convertText(Loc::getMessage($messIdmail,array('#ID#'=>$TICKET_ID,'#PRIORITY#'=>$arEvent['TICKET_PRIORITY'],'#URL#'=>$arEvent['URL']))); 
    ...
    function changeResponsible($TICKET_ID)
    {
        if($arEvent = self::getEventDataEx($TICKET_ID))
        {
            TicketMessageTable::add(Array(
                'CREATED_USER_ID' => $arEvent['TICKET_MODIFIED_USER_ID'],
                'TICKET_ID' => $TICKET_ID,
                'MESSAGE' => Loc::getMessage('ALTASIB_SUPPORT_EVENT_CHANGE_RESPONSIBLE_LOG',array('#RESPONSIBLE_TEXT#'=>$arEvent['TICKET_RESPONSIBLE_USER_LIST_NAME'])),
                'IS_LOG' => 'Y'
            ));
                        
            $arEvent['MESSAGE'] = self::convertText(Loc::getMessage('ALTASIB_SUPPORT_EVENT_CHANGE_RESPONSIBLE_MAIL',array('#ID#'=>$TICKET_ID,'#RESPONSIBLE_TEXT#'=>$arEvent['TICKET_RESPONSIBLE_USER_LIST_NAME'],'#URL#'=>$arEvent['URL']))); 
//to support
            if(Main\Loader::includeModule("im") && \CIMSettings::GetNotifyAccess($arEvent['TICKET_RESPONSIBLE_USER_ID'], self::$moduleId, 'resp', \CIMSettings::CLIENT_SITE))
            {
                //"NOTIFY_TITLE" => Loc::getMessage('ALTASIB_SUPPORT_EVENT_CHANGE_TICKET',array('#ID#'=>$TICKET_ID)),
                \CIMNotify::add(array(
                    'FROM_USER_ID' =>$arEvent['TICKET_MODIFIED_USER_ID'],
                    ...
                    'NOTIFY_MESSAGE' => Loc::getMessage('ALTASIB_SUPPORT_EVENT_CHANGE_RESPONSIBLE_SUPPORT',array('#ID#'=>$TICKET_ID,'#MESSAGE#'=>$arEvent['TICKET_MESSAGE'],'#TITLE#'=>$arEvent['TICKET_TITLE'],'#URL#'=>$arEvent['URL'])),
                    'NOTIFY_MESSAGE_OUT' => Loc::getMessage('ALTASIB_SUPPORT_EVENT_CHANGE_RESPONSIBLE_SUPPORT',array('#ID#'=>$TICKET_ID,'#MESSAGE#'=>$arEvent['TICKET_MESSAGE'],'#TITLE#'=>$arEvent['TICKET_TITLE'],'#URL#'=>$arEvent['URL'])),
                ));                                    
            }
...
//to client
            ...
            foreach($listToNotify as $TO_USER_ID)
            {
                if(Main\Loader::includeModule("im"))
                {
                    ...
                    \CIMNotify::add(array(
                    	...
                    	"NOTIFY_MODULE" => "altasib.support", 
                    	"NOTIFY_EVENT" => 'support_changeResponsible', 
                        'NOTIFY_MESSAGE' => Loc::getMessage('ALTASIB_SUPPORT_EVENT_CHANGE_RESPONSIBLE',array('#ID#'=>$TICKET_ID,'#RESPONSIBLE_TEXT#'=>$arEvent['TICKET_RESPONSIBLE_USER_LIST_NAME'],'#URL#'=>$arEvent['URL'])),
                        'NOTIFY_MESSAGE_OUT' => Loc::getMessage('ALTASIB_SUPPORT_EVENT_CHANGE_RESPONSIBLE',array('#ID#'=>$TICKET_ID,'#RESPONSIBLE_TEXT#'=>$arEvent['TICKET_RESPONSIBLE_USER_LIST_NAME'],'#URL#'=>$arEvent['URL'])),
                    ));                                    
    ...
    {
        $result = array();
        $obData = TicketTable::getList(array('filter'=>array('ID'=>$TICKET_ID),'select'=>array('OWNER_USER_ID')));
    ...
    {
        $result = array();
        $obData = TicketTable::getList(array('filter'=>array('ID'=>$TICKET_ID),'select'=>array('OWNER_USER_ID','RESPONSIBLE_USER_ID')));
        if($data = $obData->fetch())
        {
        ...
        $dataMember = TicketMemberTable::getList(array('select'=>array('USER_ID'),'filter'=>array('TICKET_ID'=>$TICKET_ID)))->fetchAll();
     ...
     * @return
     */
    function getEventDataEx($TICKET_ID,$MESSAGE_ID)
    {
        $obData = TicketTable::getList(array('filter'=>array('ID'=>$TICKET_ID),
        'select' => array('*',
    ...
    {
        $url = str_replace('#ID#',$TICKET_ID, \COption::GetOptionString('altasib.support','path_detail','',$SITE_ID));
    ...
    public static function expiredSend()
    {
        ...
        $data = TicketTable::getList(array(
            ...
            'select' => array(
                'ID','SITE_ID','DATE_CREATE','LAST_MESSAGE_USER_ID','LAST_MESSAGE_DATE','SLA_NOTICE_TIME'=>'SLA.NOTICE_TIME','SLA_RESPONSE_TIME'=>'SLA.RESPONSE_TIME'               
            ),
        ));
        while($ticket = $data->fetch())
        {
            ...
            $ts1 = \AddToTimeStamp(array('HH'=>$ticket['SLA_NOTICE_TIME']),\MakeTimeStamp($ticket['LAST_MESSAGE_DATE']->toString()));
            ...
            if($ts1<$ts2)
            {
                ...
                $dataMessage = TicketMessageTable::getList(array('order'=>array('ID'=>'DESC'),'filter'=>array('TICKET_ID'=>$ticket['ID'],'IS_HIDDEN'=>'N','IS_LOG'=>'N'),'limit'=>1,'select'=>array('MESSAGE')))->fetch();
                $arEvent['LAST_MESSAGE'] = $dataMessage['MESSAGE'];
                if(!$arEvent['LAST_MESSAGE'])
                    $arEvent['LAST_MESSAGE'] = $arEvent['MESSAGE'];
                    
                    
                $arEvent['EXPIRATION_DATE'] = date('d.m.Y H:i:s',\AddToTimeStamp(array('HH'=>$ticket['SLA_RESPONSE_TIME']),\MakeTimeStamp($ticket['LAST_MESSAGE_DATE']->toString())));
                ...
                foreach ($listToNotify as $TO_USER_ID)
                {
                    if(Main\Loader::includeModule("im"))
                    {
                        ...
                        \CIMNotify::add(array(
                            ...
                            'NOTIFY_MESSAGE' => Loc::getMessage('ALTASIB_SUPPORT_EVENT_EXPIRE_SUPPORT',array('#ID#'=>$ticket['ID'],'#TITLE#'=>$arEvent['TICKET_TITLE'],'#MESSAGE#'=>$arEvent['LAST_MESSAGE'],'#URL#'=>$arEvent['URL'])),
                            'NOTIFY_MESSAGE_OUT' => Loc::getMessage('ALTASIB_SUPPORT_EVENT_EXPIRE_SUPPORT',array('#ID#'=>$ticket['ID'],'#TITLE#'=>$arEvent['TICKET_TITLE'],'#MESSAGE#'=>$arEvent['LAST_MESSAGE'],'#URL#'=>$arEvent['URL'])),
                        ));                        
                    }
                    
                    $arEvent['MESSAGE'] = self::convertText($arEvent['LAST_MESSAGE']); 
                    
                    $forUser = \Bitrix\Main\UserTable::getList(array('filter'=>array('ID'=>$TO_USER_ID),'select'=>array('EMAIL')))->fetch();
    ...
    public static function sendAddMember($TICKET_ID,$USER_ID)
    {
        ...
        if(Main\Loader::includeModule("im"))
        {
            ...
            \CIMNotify::add(array(
                ...
                'NOTIFY_MESSAGE' => Loc::getMessage('ALTASIB_SUPPORT_EVENT_ADD_MEMBER',array('#ID#'=>$TICKET_ID,'#TITLE#'=>$data['TICKET_TITLE'],'#MESSAGE#'=>$data['TICKET_MESSAGE'],'#URL#'=>$data['URL'])),
                'NOTIFY_MESSAGE_OUT' => Loc::getMessage('ALTASIB_SUPPORT_EVENT_ADD_MEMBER',array('#ID#'=>$TICKET_ID,'#TITLE#'=>$data['TICKET_TITLE'],'#MESSAGE#'=>$data['TICKET_MESSAGE'],'#URL#'=>$data['URL'])),
            ));                                    
    ...
    {
        $fileTxt = '';
        $dataFile = FileTable::getList(array('filter'=>array('TICKET_ID'=>$TICKET_ID,'MESSAGE_ID'=>$MESSAGE_ID)));
        while($file = $dataFile->fetch())
        {
            $fileUrl = str_replace(array('#ID#','#FILE_HASH#'),array($TICKET_ID,$file['HASH']), \COption::GetOptionString('altasib.support','path_file','',$SITE_ID));
            $http = \CMain::IsHTTPS() ? "https://" : "http://";
     ...
     * @return
     */
    function setDeferred($TICKET_ID)
    {
        if($arEvent = self::getEventDataEx($TICKET_ID))
        {
            TicketMessageTable::add(Array(
    ...
    {
        $result = true;
        if(Main\Loader::includeModule("im") && !\CIMSettings::GetNotifyAccess($USER_ID, self::$moduleId, $type, $clientId))
            $result = false;
