...
class Tools
{
    ...
    function getSupportTeam()
    {
        ...
        if($arRole = $APPLICATION->GetGroupRightList(Array("MODULE_ID" => "altasib.support","G_ACCESS"=>'E'))->Fetch())
    ...
    function getCustomerList()
    {
        ...
        if($arRole = $APPLICATION->GetGroupRightList(Array("MODULE_ID" => "altasib.support","G_ACCESS"=>'C'))->Fetch())
            $arFilter["GROUPS_ID"][] = $arRole["GROUP_ID"];
    ...
    Function IsSupportTeam($USER_ID)
    {
            global $APPLICATION;
    ...
    function ShowLHE($ID,$CONTENT,$INPUT)
    {
        ...
        foreach($allowTags as $key=>$val)
        {
            ...
            if($key == 'ANCHOR' && $val=='Y')
                $controlsMap[] = array('id' => 'InsertLink',  'compact' => true, 'sort' => 210, 'wrap' => 'bx-b-link-'.$arParams["FORM_ID"]);
            if($key == 'IMG' && $val=='Y')                    
                $controlsMap[] = array('id' => 'InsertImage',  'compact' => false, 'sort' => 220);
            if($key == 'VIDEO' && $val=='Y')                    
                $controlsMap[] = array('id' => 'InsertVideo',  'compact' => true, 'sort' => 230, 'wrap' => 'bx-b-video-'.$arParams["FORM_ID"]);
            if($key == 'TABLE' && $val=='Y')                    
                $controlsMap[] = array('id' => 'InsertTable',  'compact' => false, 'sort' => 250);
            if($key == 'CODE' && $val=='Y')
                $controlsMap[] = array('id' => 'Code',  'compact' => true, 'sort' => 260);
            if($key == 'QUOTE' && $val=='Y')                    
                $controlsMap[] = array('id' => 'Quote',  'compact' => true, 'sort' => 270, 'wrap' => 'bx-b-quote-'.$arParams["FORM_ID"]);
    ...
    function taskPlannerProcess($TICKET_ID,$remove = false)
    {
        ...
        {
            $ticketData = TicketTable::getList(array('filter'=>array('ID'=>$TICKET_ID),'select'=>array('TASK_ID','RESPONSIBLE_USER_ID')))->fetch();
    ...
    function taskPlanner($TASK_ID,$USER_ID,$remove = false)
    {
        ...
        if (Main\Loader::includeModule("tasks"))
        {
            ...
            \CUserOptions::SetOption('tasks',\CTaskPlannerMaintance::PLANNER_OPTION_CURRENT_TASKS,$list,false,$USER_ID);
    ...
    public static function taskGetCurrentList($USER_ID)
    {
        if (Main\Loader::includeModule("tasks"))
        {        
            $list = \CUserOptions::GetOption('tasks', \CTaskPlannerMaintance::PLANNER_OPTION_CURRENT_TASKS, null,$USER_ID);
            if($list === null)
            {
    ...
    function __AddSocNetMenu(&$arResult)
    {
       ...
       $arResult["Urls"]["support"] = \CComponentEngine::MakePathFromTemplate(SITE_DIR."workgroups/group/#group_id#/support/", array("group_id" => $arResult["Group"]["ID"]));
    ...
    function prepareComponentParams(&$arParams,&$arResult)
    {
        ...
        if($arParams["ID"]>0)
        {
            ...
            $obTicket = TicketTable::getList(array('filter'=>array('ID'=>$arParams["ID"]),'select'=>array("ID",'CATEGORY_ID',"OWNER_USER_ID","RESPONSIBLE_USER_ID")));
            ...
            elseif($Role == "E")
            {
                if($arResult['RESPONSIBLE_USER_ID'] != $USER->GetID())
                {
                    $wtc = WtCTable::GetList(
                        array(
                            'filter' => array('USER_ID'=>$USER->GetId(),'CATEGORY_ID'=>$arResult['CATEGORY_ID'],'CLIENT_USER_ID'=>$arResult['OWNER_USER_ID'])
                        )
                    );
                    ...
                    {
                        if(!TicketMemberTable::getList(array('select'=>array('ID'),'filter'=>array('USER_ID'=>$USER->GetID(),'TICKET_ID'=>$arParams['ID'])))->fetch())
                        {
                            ShowError(GetMessage('ALTASIB_SUPPORT_CMP_TICKET_NOT_FOUND'));
                            return false;
                        }
            ...
            else
            {
                ...
                if(!$arParams['isWorker'])
                {
                    if($arResult['OWNER_USER_ID'] != $USER->GetID())
                    {
                        $c2cw = C2CWTable::GetList(
                            array(
                                'filter' => array('USER_ID'=>$USER->GetId(),'WORKER_USER_ID'=>$arResult['OWNER_USER_ID'])
                            )
                        );
                        if(!$c2cw->fetch())
                        {
                    ...
                    if($result->isSuccess())
                    {
                        if(Main\Loader::includeModule("pull") && \CPullOptions::GetNginxStatus())
                        {
                            ...
                            $dataTicketMessage = TicketMessageTable::getRow(array(
                                ...
                                'select'=>array('*','TYPE_TIME_NAME'=>'TYPE_TIME.NAME','CREATED_USER_NAME'=>'CREATED_USER.NAME','CREATED_USER_LAST_NAME'=>'CREATED_USER.LAST_NAME','CREATED_USER_SHORT_NAME'=>'CREATED_USER.SHORT_NAME')
                            ));
