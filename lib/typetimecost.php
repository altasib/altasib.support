...
Class TypeTimeCostTable extends Entity\DataManager
{
    ...
    {
        $filter = array('TYPE_TIME_ID'=>$data['TYPE_TIME_ID'],'<DATE_START'=>$data['DATE_START']->toString(),'DATE_END'=>false);
        ...
        {
            $endDate = new Type\Date(date('d.m.Y',\AddToTimeStamp(array('DD'=>-1),MakeTimeStamp($data['DATE_START']->toString(), "DD.MM.YYYY 00:00:00"))),'d.m.Y');
    ...
    public static function getCurrentCost($TYPE_ID,$USER_ID)
    {
        $ob = self::getList(array(
            'order' => array('DATE_START'=>'DESC'),
            'filter' => array('TYPE_TIME_ID'=>$TYPE_ID,'DATE_END'=>false,'USER_ID'=>$USER_ID>0 ? $USER_ID : false)
        ));
    ...
    {
        return self::getRow(array('select'=>array('ID'),'filter'=>array('USER_ID'=>$USER_ID,'TYPE_TIME_ID'=>$TYPE_ID)));
    }
    ...
    public static function getCost($USER_ID)
    {
        ...
        while($type = $data->fetch())
        {
            ...
            while($cost = $ob->fetch())
            {
                $costType['BY_TIME'][\MakeTimeStamp($cost['DATE_START']->toString(), "DD.MM.YYYY 00:00:00")] = array('COST'=>$cost['COST'],'RATE'=>$cost['RATE']);
                
                if(is_object($cost['DATE_END']) && strlen($cost['DATE_END']->toString())>0)
                {
                    if($cost['DATE_START']->toString()!=$cost['DATE_END']->toString())
                        $costType['BY_TIME'][\MakeTimeStamp($cost['DATE_END']->toString(), "DD.MM.YYYY 00:00:00")] = array('COST'=>$cost['COST'],'RATE'=>$cost['RATE']);
                }
                else
                {
