...
$arAllOptions = Array(
        "MAIN" => Array(
                ...
                Array("DEFAULT_STATUS", GetMessage("ALTASIB_SUPPORT_OPTIONS_DEFAULT_STATUS").":", "7", Array("selectbox",$arStatuss)),
                Array("RE_STATUS", GetMessage("ALTASIB_SUPPORT_OPTIONS_RE_STATUS").":", "7", Array("selectbox",$arStatuss)),
                Array("FINAL_STATUS", GetMessage("ALTASIB_SUPPORT_OPTIONS_FINAL_STATUS").":", "7", Array("selectbox",$arStatuss)),
                Array("SET_STATUS", GetMessage("ALTASIB_SUPPORT_OPTIONS_SET_STATUS").":", "7", Array("selectbox",array_merge(array('0'=>'-'),$arStatuss))),
        ...
        "ALLOW_TAGS" => Array(
                ...
                Array("FORM_ALLOW_ANCHOR", GetMessage("ALTASIB_SUPPORT_OPTIONS_FORM_ALLOW_ANCHOR").":", "Y", Array("checkbox")),
...
$arDefaultPathValues = array(
   ...
   if ($REQUEST_METHOD=="POST" && strlen($Update)>0 && $RF_RIGHT=="W" && check_bitrix_sessid())
   {
        ...
        foreach($arSiteList as $site)
        {
        	foreach($arDefaultPathValues as $key=>$value)
        	{
        		...
        		{
        			COption::SetOptionString("altasib.support", $key, $_POST[$key."_".$site["LID"]], false, $site["LID"]);
        		}
        	}
        }           
   ...
   ?>
                   <tr>
                           ...
                           if($type[0]=="checkbox"):
                                   ?><input type="checkbox" id="<?echo htmlspecialcharsBx($Option[0])?>" name="<?echo htmlspecialcharsBx($Option[0])?>" value="Y"<?if($val=="Y")echo" checked";?>><?
                           elseif($type[0]=="text"):
                                   ?><input type="text" size="<?echo $type[1]?>" maxlength="255" value="<?=htmlspecialcharsBx($val)?>" name="<?echo htmlspecialcharsBx($Option[0])?>"><?
...
$aTabs = array(
        array("DIV" => "main", "TAB" => GetMessage("ALTASIB_SUPPORT_OPTIONS_MAIN"), "ICON" => "references_settings", "TITLE" => GetMessage("ALTASIB_SUPPORT_OPTIONS_MAIN_TITLE")),
        array("DIV" => "rights", "TAB" => GetMessage("MAIN_TAB_RIGHTS"), "ICON" => "references_settings", "TITLE" => GetMessage("MAIN_TAB_TITLE_RIGHTS")),
        );
$tabControl = new CAdminTabControl("tabControl", $aTabs);
$tabControl->Begin();
?>
<form method="POST" name="opt_ir" id="opt_ir" action="<?echo $APPLICATION->GetCurPage()?>?mid=<?=htmlspecialcharsBx($mid)?>&amp;lang=<?echo LANG?>">
...
<?ShowParamsHTMLByArray($arAllOptions["ALLOW_TAGS"]);?>
    ...
    <tr>
		...
		<?$subTabControl->BeginNextTab();?>
            ...
            <?foreach($arDefaultPathValues as $key=>$value):if($site['DIR']!='/')$value=$site['DIR'].substr($value,1);?>
        		<tr>
        			<td align="right"><?=GetMessage("ALTASIB_SUPPORT_".strtoupper($key))?>:</td>
        			<td><input type="text" size="40" value="<?=COption::GetOptionString("altasib.support", $key, $value, $site["LID"])?>" name="<?=$key?>_<?=htmlspecialcharsbx($site["LID"]); ?>"></td>
        		</tr>
            <?endforeach;?>
        	</table>
		<?endforeach;?>
...
<input type="submit" <?if ($RF_RIGHT<"W") echo "disabled" ?> name="Update" value="<?echo GetMessage("MAIN_SAVE")?>">
<input type="reset" name="reset" onClick="document.getElementById('site_select_id').disabled=<? if(COption::GetOptionString($module_id, "different_set", "N") != "Y") echo "true"; else echo "false"; ?>; SelectSite('<?echo htmlspecialcharsBx($siteList[0]["ID"])?>');" value="<?echo GetMessage("MAIN_RESET")?>">
