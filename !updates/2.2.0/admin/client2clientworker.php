...
if ($arID = $adminList->GroupAction()) {
    ...
    foreach ($arID as $ID) {
        ...
        switch ($_REQUEST['action']) {
            case "delete":
                Support\C2CWTable::deleteByKey($ID);
                break;
        }
    }
}
...
$adminList->AddHeaders(array(
    array(
        "id" => "USER_ID",
        "content" => Loc::getMessage("ALTASIB_SUPPORT_CONN_CLIENT2CLIENTWORKER_LIST_CLIENT"),
        "default" => true
    ),
    array(
        "id" => "WORKER_USER_ID",
        "content" => Loc::getMessage("ALTASIB_SUPPORT_CONN_CLIENT2CLIENTWORKER_LIST_CLIENT_WORKER"),
        "default" => true
    ),
...
$data = Support\C2CWTable::getList(array(
    ...
    'select' => array(
        ...
        'WORKER_USER_SHORT_NAME' => 'WORKER_USER.SHORT_NAME'
    ),
    'group' => array('USER_ID')
));
$data = new CAdminResult($data, $sTableID);
$data->NavStart();
$adminList->NavText($data->GetNavPrint(Loc::getMessage('ALTASIB_SUPPORT_CONN_CLIENT2CLIENTWORKER_NAV_TITLE')));
while ($c2cw = $data->Fetch()) {
    ...
    $row =& $adminList->AddRow($c2cw['USER_ID'].'-'.$c2cw['WORKER_USER_ID'], $c2cw, $EditLink);
