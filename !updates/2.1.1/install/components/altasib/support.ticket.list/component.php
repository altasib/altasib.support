...
$arParams["CREATE_URL"] = htmlspecialchars(CComponentEngine::MakePathFromTemplate($arParams["URL_DETAIL"], Array("ID" => "0","TICKET_ID"=>0,"CODE"=>0,'group_id'=>$arParams['GROUP_ID'])));
...
$arParams["HAS_CREATE"] = $arParams['SUPPORT_TEAM'] = $arParams['Right']->isSupportTeam();
    ...
    $arResult = Array();
    $gridFilter = array();
    $f = Support\TicketTable::getUserRoleFilter();
    ...
    $arParams['GRID']['HEADER'] = array(
        array("id"=>"ID", "name"=>$arParams['TICKET_MAP']['ID']['title'], "sort"=>"ID", "default"=>true, "editable"=>false),
        array("id"=>"DATE_CREATE", 'type'=>'date', "name"=>$arParams['TICKET_MAP']['DATE_CREATE']['title'], "sort"=>"DATE_CREATE", "default"=>false, "editable"=>false,'align'=>'center'),
        array("id"=>"TIMESTAMP", 'type'=>'date', "name"=>$arParams['TICKET_MAP']['TIMESTAMP']['title'], "sort"=>"TIMESTAMP", "default"=>false, "editable"=>false,'align'=>'center'),
        array("id"=>"IS_CLOSE", 'type'=>'checkbox', "name"=>$arParams['TICKET_MAP']['IS_CLOSE']['title'], "sort"=>"IS_CLOSE", "default"=>false, "editable"=>false,'align'=>'center'),
        array("id"=>"DATE_CLOSE", 'type'=>'date', "name"=>$arParams['TICKET_MAP']['DATE_CLOSE']['title'], "sort"=>"DATE_CLOSE", "default"=>false, "editable"=>false,'align'=>'center'),
        array("id"=>"PRIORITY", "name"=>$arParams['TICKET_MAP']['PRIORITY_ID']['title'], "sort"=>"PRIORITY_ID", "default"=>false, "editable"=>false),
        array("id"=>"STATUS", "name"=>$arParams['TICKET_MAP']['STATUS_ID']['title'], "sort"=>"STATUS_ID", "default"=>true, "editable"=>false),
        array("id"=>"TITLE", "name"=>$arParams['TICKET_MAP']['TITLE']['title'], "sort"=>"TITLE", "default"=>true, "editable"=>false),
        array("id"=>"CATEGORY", "name"=>$arParams['TICKET_MAP']['CATEGORY_ID']['title'], "sort"=>"CATEGORY_ID", "default"=>true, "editable"=>false),
        array("id"=>"STATUS", "name"=>$arParams['TICKET_MAP']['STATUS_ID']['title'], "sort"=>"STATUS_ID", "default"=>true, "editable"=>false),
        array("id"=>"LAST_MESSAGE_DATE", 'type'=>'date', "name"=>$arParams['TICKET_MAP']['LAST_MESSAGE_DATE']['title'], "sort"=>"LAST_MESSAGE_DATE", "default"=>true, "editable"=>false,'align'=>'center'),
        array("id"=>"MESSAGE_CNT", "name"=>$arParams['TICKET_MAP']['MESSAGE_CNT']['title'], "sort"=>"MESSAGE_CNT", "default"=>true, "editable"=>false,'align'=>'center'),
    );
    if(IsModuleInstalled('socialnetwork'))
    {
        if($arParams['GROUP_ID']==0)
            $arParams['GRID']['HEADER'][] =array("id"=>"GROUP_ID", "name"=>GetMessage('ALTASIB_STL_T_LIST_GROUP_ID'), "sort"=>"GROUP_ID", "default"=>true, "editable"=>false);
    }     
    $arParams['GRID']['HEADER'][] = array("id"=>"CREATED_USER", "name"=>$arParams['TICKET_MAP']['CREATED_USER_ID']['title'], "sort"=>"CREATED_USER_ID", "default"=>false, "editable"=>false);
    $arParams['GRID']['HEADER'][] = array("id"=>"LAST_MESSAGE_USER", "name"=>$arParams['TICKET_MAP']['LAST_MESSAGE_USER_ID']['title'], "sort"=>"LAST_MESSAGE_USER_ID", "default"=>false, "editable"=>false);
    if($arParams['SUPPORT_TEAM'])
    {
        $arParams['GRID']['HEADER'][] = array("id"=>"MODIFIED_USER", "name"=>$arParams['TICKET_MAP']['MODIFIED_USER_ID']['title'], "sort"=>"MODIFIED_USER_ID", "default"=>false, "editable"=>false);
        $arParams['GRID']['HEADER'][] = array("id"=>"OWNER", "name"=>$arParams['TICKET_MAP']['OWNER_USER_ID']['title'], "sort"=>"OWNER_USER_ID", "default"=>true, "editable"=>false);    
        $arParams['GRID']['HEADER'][] = array("id"=>"RESPONSIBLE", "name"=>$arParams['TICKET_MAP']['RESPONSIBLE_USER_ID']['title'], "sort"=>"RESPONSIBLE_USER_ID", "default"=>true, "editable"=>false);
        $arParams['GRID']['HEADER'][] = array("id"=>"SLA", "name"=>$arParams['TICKET_MAP']['SLA_ID']['title'], "sort"=>"SLA_ID", "default"=>false, "editable"=>false);
        $arParams['GRID']['HEADER'][] = array("id"=>"IP", "name"=>$arParams['TICKET_MAP']['IP']['title'], "default"=>false);
    }
        $setLogic = false;
        if($arParams["SHOW_FILTER"])
        {
        	...
        	array('id'=>'ID', 'name'=>'ID','default'=>true),
                array('id'=>'IS_CLOSE','name'=>$arParams['TICKET_MAP']['IS_CLOSE']['title'],'type'=>'list','items'=>$isOpenFilter,'filtered'=>(!isset($_REQUEST['IS_CLOSE'])),'filter_value'=>'N'),
                ...
                array('id'=>'CATEGORY_ID','name'=>$arParams['TICKET_MAP']['CATEGORY_ID']['title'],'type'=>'list','items'=>$filterCategory),
                array('id'=>'STATUS_ID','name'=>$arParams['TICKET_MAP']['STATUS_ID']['title'],'type'=>'list','items'=>$filterStatus),
                array('id'=>'NO_REPLY','name'=>GetMessage('ALTASIB_SUPPORT_FILTER_NO_REPLY'),'type'=>'checkbox','value'=>'Y'),
                array('id'=>'TIMESTAMP', 'type'=>'date', "name"=>$arParams['TICKET_MAP']['TIMESTAMP']['title'],'default'=>false),
            );
            if(\Bitrix\Main\Loader::includeModule('socialnetwork'))
            {
                if($arParams['GROUP_ID']==0)
                {
                    ...
                    $arResult['GRID_FILTER'][] = array('id'=>'GROUP_ID', 'name'=>GetMessage('ALTASIB_STL_T_LIST_GROUP_ID'),'type'=>'list','items'=>$groupList);
                }
            }

            if($arParams['SUPPORT_TEAM'])
                $arResult['GRID_FILTER'][] = array('id'=>'OWNER_USER_ID', 'name'=>$arParams['TICKET_MAP']['OWNER_USER_ID']['title']);
            ...
            if($arParams['Right']->getRole()==ALTASIB\Support\Rights::ADMIN)
            {
                ...
                $arResult['GRID_FILTER'][] = array('id'=>'RESPONSIBLE_USER_ID', 'name'=>$arParams['TICKET_MAP']['RESPONSIBLE_USER_ID']['title'],'type'=>'list','items'=>$supportTeam);
            }
        ...
        if (!empty($request["clear_filter"]) && $arParams['SHOW_FILTER'])
            LocalRedirect($APPLICATION->GetCurPageParam('',array('setFilter','STATUS_ID','CLOSE','TITLE','TICKET_MESSAGE','MESSAGE','CATEGORY_ID','clear_filter')));
        
        if(\Bitrix\Main\ModuleManager::isModuleInstalled('intranet') && $arParams['SHOW_MENU_COUNTER']=='Y')
        {
            $cntFilter = array_merge($filter,array('IS_CLOSE'=>'N','LAST_MESSAGE_BY_SUPPORT'=>($arParams['IS_SUPPORT_TEAM'] ? 'N':'Y')));
        ...
        if($arParams['Right']->getRole()==ALTASIB\Support\Rights::SUPPORT)
        {
            ...
            $dataMember = Support\TicketMemberTable::getList(array('select'=>array('TICKET_ID'),'filter'=>$memberFilter))->fetchAll();
        ...
        $parametrs = array(
            'order' => $order,
            'filter' => $filter,
            'select' => array('*','CATEGORY_NAME'=>'CATEGORY.NAME','STATUS_NAME'=>'STATUS.NAME','OWNER_USER','CREATED_USER','MODIFIED_USER','MESSAGE_CNT',
        ...
        while($arTicket = $result->fetch())
        {
            ...
            $arTicket["URL_DETAIL"] = str_replace(Array("#ID#","#TICKET_ID#","#CODE#",'#group_id#'),Array($arTicket["ID"],$arTicket["ID"],$arTicket["CODE"],$arParams['GROUP_ID']),$arParams["URL_DETAIL"]);
