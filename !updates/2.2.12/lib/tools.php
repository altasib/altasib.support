...
class Tools
{
    ...
    public static function getSupportTeam()
    {
        ...
        $arResult = array(/*"0"=>"-"*/);
        $arFilter["ACTIVE"] = "Y";
        $arFilter["GROUPS_ID"] = self::getSupportGroup();
    ...
    public static function getSupportAdmins()
    {
        global $APPLICATION;
        $arResult = array();
        $arFilter = ["ACTIVE" => "Y"];

        if ($arRole = $APPLICATION->GetGroupRightList(array("MODULE_ID" => "altasib.support", "G_ACCESS" => 'W'))
            ->Fetch()) {
    ...
    public static function getSupportAdminTeam()
    {
        ...
        $obRole = $APPLICATION->GetGroupRightList(array(
                ...
                if ($ticketId > 0) {
                    ...
                    if ($result->isSuccess()) {
                        if (Main\Loader::includeModule("pull") && \CPullOptions::GetNginxStatus()) {
                            ...
                            \CPullWatch::AddToStack('ALTASIB_SUPPORT_' . $ticketId, array(
                                'module_id' => 'altasib.support',
                                'command' => 'message',
                                'params' => $pullParams
                            ));
                            ...
                            $pullParams['MESSAGE'] = \getMessageSupport($dataTicketMessage, $arParams, $dataTicket);
                            \CPullWatch::AddToStack('ALTASIB_SUPPORT_' . $ticketId . '_SUPPORT', array(
                                'module_id' => 'altasib.support',
                                'command' => 'message',
                                'params' => $pullParams
                            ));
                            ...
                            $pullParams['MESSAGE'] = \getMessageSupport($dataTicketMessage, $arParams, $dataTicket);
                            \CPullWatch::AddToStack('ALTASIB_SUPPORT_' . $ticketId . '_SUPPORT_ADMIN', array(
                                'module_id' => 'altasib.support',
                                'command' => 'message',
                                'params' => $pullParams
                            ));
    ...
    function getCustomerList()
    {
        ...
        $obRole = $APPLICATION->GetGroupRightList(array("MODULE_ID" => "altasib.support", "G_ACCESS" => 'C'));
        while ($arRole = $obRole->Fetch()) {
            $arFilter["GROUPS_ID"][] = $arRole["GROUP_ID"];
        }
